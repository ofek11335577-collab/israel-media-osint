...
def extract_real_image(entry):
    """חילוץ תמונה אמיתית מהפיד (media_content, enclosures או תמונות מתוך התוכן)"""
    # 1. בדיקת media_content
    if hasattr(entry, 'media_content') and entry.media_content:
        for m in entry.media_content:
            if 'url' in m and ('image' in m.get('type', '') or m.get('url', '').endswith(('jpg', 'jpeg', 'png'))):
                return m['url']
    ...
    # 3. חיפוש תמונה בתוך ה-summary / content
    summary = entry.get('summary', '') or entry.get('description', '')
    img_match = re.search(r'src=["\'](https?://[^"\']+\.(?:jpg|jpeg|png))["\']', summary, re.I)
    if img_match:
